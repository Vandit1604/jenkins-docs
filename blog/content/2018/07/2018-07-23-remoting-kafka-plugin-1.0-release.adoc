...
image:/post-images/remoting-kafka/remoting-kafka-architecture.png[title="Remoting Kafka Plugin Current Architecture", role="center"]
...
image:/post-images/remoting-kafka/remoting-kafka-configuration.png[title="Remoting Kafka Plugin Global Configuration", role="center"]

=== 2. Launch agent with Kafka Launcher.
image:/post-images/remoting-kafka/launch-agent-kafka.png[title="Launch agent with Kafka Launcher", role="center"]

=== 3. Launch agent from CLI using agent JAR with secret provided to ensure security.
image:/post-images/remoting-kafka/agent-cli.png[title="Command to run agent in CLI", role="center"]

=== 4. Run jobs, pipeline using Kafka agent.
image:/post-images/remoting-kafka/demo-jobs.png[title="Run jobs, pipeline using Kafka agent", role="center"]

=== 5. Kafka communication between controller and agent.
image:/post-images/remoting-kafka/kafka-commands.png[title="Kafka communication between controller and agent", role="center"]
