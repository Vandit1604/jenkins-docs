...
. Credits for reporters xref:security:ROOT:reporting.adoc[who informed the Jenkins project]
...
Learn more: xref:security:ROOT:for-administrators.adoc#later[Are Security Advisories Updated Later?]
...
In many cases, security fixes also include xref:security:ROOT:for-administrators.adoc#escape-hatches[hidden options that allow you to disable (parts of) security fixes temporarily if they turn out to cause problems].

If you're unable to apply every security update immediately, security advisories will explain for every security issue:

* xref:security:ROOT:for-administrators.adoc#attacker[How can the issue be exploited, and by whom?]
* xref:security:ROOT:for-administrators.adoc#impact[What is the impact when exploited?]
...
Learn more:
xref:dev-docs:security:plugin.adoc#followup[Handling Vulnerabilities in Plugins: Following Up Later]
