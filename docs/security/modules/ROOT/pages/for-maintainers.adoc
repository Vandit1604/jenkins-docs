...
For plugins with xref:publishing:releasing-manually/[manual release process] this is the `prepare for next development iteration` commit.
For plugins delivered with xref:publishing:releasing-cd.adoc[JEP-229 CD automated releases]), it is the commit that got tagged as a release.
...
For plugins with xref:publishing:releasing-manually.adoc[manual release process], you could set the version number used for releases from the backport branch at this point, for example:
...
For plugins delivered with xref:publishing:releasing-cd.adoc[JEP-229 CD automated releases], you also need to manually specify a custom version number for the backport branch.
...
** xref:xss-prevention.adoc[Preventing Cross-Site Scripting in Jelly views]
