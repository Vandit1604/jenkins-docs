...
. *Random*: The TCP port is chosen at random to avoid collisions on the Jenkins xref:user-docs:glossary:index.adoc#controller[controller].
...
See xref:user-docs:security:markup-formatter.adoc[Markup Formatter].


== CSRF Protection

See xref:user-docs:security:csrf-protection.adoc[CSRF Protection].

== Agent/Master Access Control

See xref:user-docs:security:controller-isolation.adoc[Isolating the Controller from Builds].
