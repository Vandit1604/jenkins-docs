...
See xref:dev-docs:security:access-control/permissions.adoc#administer[the documentation on the access given to administrators] about the impact of unintentionally granting Administer permission.
...
Built-in node::
Users with limited permissions xref:dev-docs:security:controller-isolation.adoc[must not be able to configure jobs that run on the built-in node].
...
With this permission, users can upload and install plugins and have access to the xref:user-docs:managing:script-console.adoc[Script Console].
...
For more details about the various permissions in Jenkins and the level of access they grant, see xref:dev-docs:security:access-control/permissions.adoc[Permissions].

== Disabling Access Control

See xref:dev-docs:security:access-control/disable.adoc[Disable Access Control].
