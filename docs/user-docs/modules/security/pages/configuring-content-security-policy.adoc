...
* *Are some agents not fully trusted?* Even when xref:dev-docs:security:controller-isolation.adoc#agent-controller-access-control[Agent To Controller Access] is used to limit what agents can do on the
...
The CSP header sent by Jenkins can be modified by xref:user-docs:managing:system-properties.adoc[setting the Java system property] `hudson.model.DirectoryBrowserSupport.CSP`:
