...
* *New Pipeline* option from the xref:user-docs:blueocean:dashboard.adoc[Blue Ocean dashboard].
* *Branches* tab within the xref:user-docs:blueocean:activity.adoc#branches[Activity view].
...
Selecting an existing stage or adding a stage displays the xref:user-docs:blueocean:pipeline-editor.adoc#stage-configuration[stage configuration] pane instead.
...
Selecting *Save & run* saves any changes to the Pipeline as a new commit, starts a new Pipeline run based on those changes, and navigates to the xref:user-docs:glossary:index.adoc#activity[Activity view] for this pipeline.
