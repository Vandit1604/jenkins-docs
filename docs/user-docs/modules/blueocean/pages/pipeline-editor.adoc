...
To use the editor, a user must first xref:#creating-pipelines[ create a Pipeline in Blue Ocean] or have at least one existing Pipeline in Jenkins.
...
Selecting *Save & run* saves any changes to the Pipeline as a new commit, starts a new Pipeline run based on those changes, and navigates to the xref:glossary:index.adoc#activity[Activity view] for this pipeline.
