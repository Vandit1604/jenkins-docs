...
See
xref:dev-docs:security:controller-isolation.adoc[Distributed Builds]
in the
...
Learn more about Agents in xref:user-docs:managing:nodes/.adoc#creating-agents[Managing Nodes]
...
** The System Property xref:user-docs:managing:system-properties.adoc#jenkins-model-jenkins-buildsdir[jenkins.model.Jenkins.buildsDir]
...
See the xref:user-docs:system-administration:backing-up.adoc[Backup policies] for more details.
...
Note that starting with Jenkins 2.119, the User Interface for this was replaced by the `jenkins.model.Jenkins.buildsDir` system property. See the xref:user-docs:managing:system-properties.adoc[dedicated _Features Controlled with System Properties_ wiki page] for more details.
