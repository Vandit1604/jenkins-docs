...
To install a Jenkins controller, create your docker image based on the base Jenkins image, which can be found in the official docker repository (keep in mind that Docker should be installed on your local machine, see the installation steps xref:installing-jenkins:docker.adoc#installing-docker[here]).
...
After successfully creating your Jenkins image, follow the steps explained in the xref:installing-jenkins:kubernetes.adoc#install-jenkins-with-yaml-files[Jenkins installation doc] to deploy your Jenkins controller.
