...
Java 11 Docker installation instructions are included in xref:installing-jenkins:index.adocdocker/#downloading-and-running-jenkins-in-docker["Downloading and running Jenkins in Docker"].
...
* xref:publishing:releasing-cd.adoc[Automated plugin releases]
