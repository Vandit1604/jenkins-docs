...
If you don't have Helm command line interface installed and configured locally, see the sections below to <<install-helm,Install Helm>> and <<configure-helm,Configure Helm>>.
...
Visit the xref:projects:ROOT:index.adoc/jcasc/[Jenkins Configuration as Code project] for more information on configuration as code.
