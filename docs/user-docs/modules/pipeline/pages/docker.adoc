...
xref:glossary:index.adoc#stage[Stage]
...
xref:glossary:index.adoc#agent[agent]
...
xref:pipeline:syntax.adoc#agent[Pipeline Syntax] section.
...
xref:glossary:index.adoc#agent[agent] is capable of running Docker-based Pipelines.
...
the
xref:glossary:index.adoc#folder[Folder]
level, for specifying which agents (by
xref:glossary:index.adoc#label[Label])
