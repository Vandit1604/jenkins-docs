...
Pipeline is designed to easily use link:https://docs.docker.com/[Docker] images as the execution environment for a single xref:glossary:index.adoc#stage[Stage] or the entire Pipeline.
...
Pipeline supports adding custom arguments that are passed to Docker, allowing users to specify custom link:https://docs.docker.com/engine/tutorials/dockervolumes/[Docker Volumes] to mount, which can be used for caching data on the xref:glossary:index.adoc#agent[agent] between Pipeline runs.
...
The `agent { dockerfile true }` syntax supports a number of other options, which are described in more detail in the xref:pipeline:syntax.adoc#agent[Pipeline Syntax] section.
...
By default, Pipeline assumes that _any_ configured xref:glossary:index.adoc#agent[agent] is capable of running Docker-based Pipelines.
...
Pipeline provides a global option on the *Manage Jenkins* page and on the xref:glossary:index.adoc#folder[Folder] level, for specifying which agents (by xref:glossary:index.adoc#label[Label]) to use for running Docker-based Pipelines.
