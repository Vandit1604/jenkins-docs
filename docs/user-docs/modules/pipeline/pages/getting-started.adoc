...
As mentioned xref:pipeline:index.adoc[previously], Jenkins Pipeline is a suite of
...
Both
xref:pipeline:index.adoc#declarative-versus-scripted-pipeline-syntax[Declarative and Scripted Pipeline]
...
+
[.boxshadow]
image::new-item-creation.png[alt="Enter a name, click *Pipeline* and then click *OK*",width=100%]
...
+
[.boxshadow]
image::example-pipeline-in-classic-ui.png[alt="Example Pipeline in the classic UI",width=100%]
+
...
+
[.boxshadow]
image::classic-ui-left-column-on-item.png[alt="Classic UI left column on an item",width=35%]
...
+
[.boxshadow]
image::hello-world-console-output.png[alt="*Console Output* for the Pipeline",width=70%]
+
*Notes:*
...
[.boxshadow]
image::snippet-generator.png[alt="Snippet Generator",width=100%]
...
does not cover the xref:pipeline:syntax.adoc#sections[sections] and
xref:pipeline:syntax.adoc#directives[directives] used to define a Declarative Pipeline.
...
In the next section, xref:user-docs:glossary:index.adoc#jenkinsfile[The Jenkinsfile], more Pipeline steps
