...
As discussed at the xref:pipeline:syntax.adoc[start of this chapter], the most fundamental part of a Pipeline is the "step". Basically, steps tell Jenkins _what_ to do and serve as the basic building block for both Declarative and Scripted Pipeline syntax.
...
* Blocks must only consist of xref:#declarative-sections>>[ <<declarative-directives], <<declarative-steps>>, or assignment statements.
...
| Parameters
| xref:#agent-parameters[ Described below]
...
docker:: Execute the Pipeline, or stage, with the given container which will be dynamically provisioned on a xref:#../glossary#node[ node] pre-configured to accept Docker-based Pipelines, or on a node matching the optionally defined `label` parameter. 
...
----
<1> Defining `agent none` at the top-level of the Pipeline ensures that xref:#../glossary#executor[ an Executor] will not be assigned unnecessarily.
...
<2> xref:#post-conditions[ Post-condition] blocks contain xref:#declarative-steps[ steps] the same as the <<steps>> section.
=====
...
`script` blocks of non-trivial size and/or complexity should be moved into xref:#shared-libraries#[ Shared Libraries] instead.
...
In order to provide _durability_, which means that running Pipelines can survive a restart of the Jenkins xref:#../glossary#controller[ controller], Scripted Pipeline must serialize data back to the controller.
...
Both are able to utilize xref:#shared-libraries#[ Shared Libraries]
