...
xref:getting-started:index.adoc[Getting started with Pipeline] and should be treated
solely as a reference. For more information on how to use Pipeline syntax in
practical examples, refer to the
xref:pipeline:jenkinsfile.adoc[Using a Jenkinsfile] section of this chapter. As of version
...
As discussed at the xref:pipeline:syntax.adoc[start of this chapter], the most fundamental part
...
You can use the
xref:getting-started:index.adoc#directive-generator[Declarative Directive Generator]
...
dynamically provisioned on a xref:user-docs:glossary:index.adoc#node[ node] pre-configured to
...
xref:user-docs:glossary:index.adoc#executor[ an Executor] will not be assigned unnecessarily.
...
into xref:user-docs:glossary:index.adoc#shared-libraries[Shared Libraries] instead.
...
survive a restart of the Jenkins xref:user-docs:glossary:index.adoc#controller[ controller], Scripted
...
xref:user-docs:glossary:index.adoc#shared-libraries[Shared Libraries]
