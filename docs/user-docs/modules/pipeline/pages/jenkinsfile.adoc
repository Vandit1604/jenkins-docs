...
The xref:#syntax#stages[ stages directive], and xref:#syntax#steps[ steps directives]
...
Declarative Pipeline supports an xref:#syntax#environment[ environment]
...
within the xref:#syntax#environment[`environment`] directive) which supports
...
Pipeline's xref:#syntax#environment[`environment`] directive), within this stage's
...
xref:#syntax#environment[`environment`] directive) are available within this
...
xref:#syntax#post[ post section] which allows declaring a number of different
"post conditions" such as: `always`, `unstable`, `success`, `failure`, and
`changed`. The xref:#syntax[ Pipeline Syntax] section provides more detail on
...
expression. Consult the xref:#syntax#[ Pipeline Syntax] section for more details.
