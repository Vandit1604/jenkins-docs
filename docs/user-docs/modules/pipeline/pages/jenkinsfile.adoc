...
xref:getting-started:index.adoc[Getting started with Pipeline]
...
Pipeline supports xref:pipeline:syntax.adoc[two syntaxes], Declarative (introduced in
...
As discussed in the
xref:getting-started:index.adoc#defining-a-pipeline-in-scm[Defining a Pipeline in SCM],
...
The xref:./syntax#stages, stages directive>>, and <<syntax.adoc#steps[steps directives]
...
Declarative Pipeline supports an xref:./syntax.adoc#environment[environment]
...
xref:./syntax.adoc#post[post section] which allows declaring a number of different
...
expression. Consult the xref:user-docs:glossary:index.adoc#syntax[Pipeline Syntax] section for more details.
