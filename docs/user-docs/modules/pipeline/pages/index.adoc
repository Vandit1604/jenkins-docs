...
* xref:user-docs:blueocean:getting-started.adoc[get started with Pipeline] - covers how to
  xref:user-docs:pipeline:getting-started.adoc#defining-a-pipeline[define a Jenkins Pipeline] (i.e. your
  `Pipeline`) through
  xref:user-docs:pipeline:getting-started.adoc#through-blue-ocean[Blue Ocean], through the
  xref:user-docs:pipeline:getting-started.adoc#through-the-classic-ui[classic UI] or in
  xref:user-docs:pipeline:getting-started.adoc#defining-a-pipeline-in-scm[SCM],
* xref:user-docs:pipeline:jenkinsfile.adoc[create and use a `Jenkinsfile`] - covers use-case scenarios
  on how to craft and construct your `Jenkinsfile`,
* work with xref:user-docs:pipeline:multibranch.adoc[branches and pull requests],
* xref:user-docs:pipeline:docker.adoc[use Docker with Pipeline] - covers how Jenkins can invoke Docker
  containers on agents/nodes (from a `Jenkinsfile`) to build your Pipeline
  projects,
* xref:user-docs:pipeline:shared-libraries.adoc[extend Pipeline with shared libraries],
* use different xref:user-docs:pipeline:development.adoc[development tools] to facilitate the creation
  of your Pipeline, and
* work with xref:user-docs:pipeline:syntax.adoc[Pipeline syntax] - this page is a comprehensive
...
xref:user-docs:pipeline:syntax.adoc[Pipeline domain-specific language (DSL) syntax].
...
xref:user-docs:pipeline:jenkinsfile.adoc[`Jenkinsfile`]) which in turn can be committed to a project's
...
both by users with xref:user-docs:pipeline:shared-libraries.adoc[Pipeline Shared Libraries] and by
plugin developers.
...
to Pipeline syntax (see the xref:user-docs:pipeline:index.adoc#pipeline-syntax-overview[overview] below).
...
xref:user-docs:pipeline:index.adoc#declarative-pipeline-fundamentals[key part of Declarative Pipeline syntax].
...
xref:user-docs:pipeline:index.adoc#scripted-pipeline-fundamentals[key part of Scripted Pipeline syntax].
...
between xref:user-docs:pipeline:index.adoc#declarative-pipeline-fundamentals[Declarative Pipeline syntax] and
xref:user-docs:pipeline:index.adoc#scripted-pipeline-fundamentals[Scripted Pipeline syntax].

Be aware that both xref:user-docs:pipeline:index.adoc#stage[stages] and xref:user-docs:pipeline:index.adoc#step[steps] (above) are common
...
----
<1> xref:user-docs:pipeline:syntax.adoc#declarative-pipeline[`pipeline`] is Declarative
Pipeline-specific syntax that defines a "block" containing all content and
instructions for executing the entire Pipeline.
<2> xref:user-docs:pipeline:syntax.adoc#agent[`agent`] is Declarative Pipeline-specific syntax that
instructs Jenkins to allocate an executor (on a node) and workspace for the
entire Pipeline.
<3> `stage` is a syntax block that describes a
xref:user-docs:pipeline:index.adoc#stage[stage of this Pipeline]. Read more about `stage` blocks in
Declarative Pipeline syntax on the xref:user-docs:pipeline:syntax.adoc#stage[Pipeline syntax] page. As
mentioned xref:user-docs:pipeline:index.adoc#scripted-pipeline-fundamentals[above], `stage` blocks are
optional in Scripted Pipeline syntax.
<4> xref:user-docs:pipeline:syntax.adoc#steps[`steps`] is Declarative Pipeline-specific syntax that
describes the steps to be run in this `stage`.
<5> `sh` is a Pipeline xref:user-docs:pipeline:syntax.adoc#steps[step] (provided by the
{plugin}workflow-durable-task-step[Pipeline: Nodes and Processes plugin]) that
executes the given shell command.
<6> `junit` is another Pipeline xref:user-docs:pipeline:syntax.adoc#steps[step] (provided by the
{plugin}junit[JUnit plugin]) for aggregating test reports.
<7> `sh` is a Pipeline xref:user-docs:pipeline:syntax.adoc#steps[step] (provided by the
{plugin}workflow-durable-task-step[Pipeline: Nodes and Processes plugin]) that
executes the given shell command.

Read more about Pipeline syntax on the xref:user-docs:pipeline:syntax.adoc[Pipeline Syntax] page.
