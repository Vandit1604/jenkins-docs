...
Jenkins project includes multiple sub-projects (including xref:ROOT:index.adoc/jcasc/[Configuration-as-Code], xref:ROOT:index.adoc/infrastructure/[Infrastructure] and xref:ROOT:index.adoc/remoting/[Remoting]) and xref:sigs:ROOT:index.adoc[special interest groups].
...
Before submitting please go through the link:https://google.github.io/gsocguides/student/[GSoC contributor guide] and the xref:ROOT:index.adoc/gsoc/students/[Jenkins GSoC contributor guide] which documents Jenkins specific requirements. Participating in Google Summer of Code requires 15-20 hours commitment a week over several months. If it may overlap with your study, internship, work or other commitments, we recommend you plan accordingly.
...
* We have explicit expectations from mentors, they are documented in our xref:ROOT:index.adoc/gsoc/mentors[mentor guidelines].
