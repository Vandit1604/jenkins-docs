...
NOTE: If you want to adopt a plugin, please see xref:plugin-governance:adopt-a-plugin.adoc[Adopt a Plugin] instead.
...
. Via the ChatOps command in the `#jenkins` IRC channel, if you have voice permissions there (see the xref:projects:ROOT:index.adoc/infrastructure/ircbot/[IRCBot])
...
If there no response from a maintainer, a xref:plugin-governance:adopt-a-plugin.adoc[Plugin Adoption Process] might be used to request ownership of the plugin.

There are two following ways to do that:

. Via ChatOps command in the `#jenkins` IRC channel, if you have voice permissions there (see the xref:projects:ROOT:index.adoc/infrastructure/ircbot/[IRCBot])
. Via a request in the link:/mailing-lists[developer mailing list].
...
Permission removal can be requested in the same way, e.g. during xref:plugin-governance:adopt-a-plugin.adoc[plugin adoption].
