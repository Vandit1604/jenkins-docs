...
As discussed in xref:controller-isolation.adoc[Controller Isolation], Jenkins should be set up for _distributed builds_ to ensure build scripts are not executing on the controller, where they could access the Jenkins home directory or otherwise interfere with the operation of Jenkins.
...
xref:environment-variables.adoc[Handling Environment Variables] discusses this topic in detail, including solutions.
