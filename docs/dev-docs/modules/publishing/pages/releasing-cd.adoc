...
NOTE: Continuous Delivery of Jenkins Components and Plugin ({jep}229/README.adoc[]) is relatively new.
...
If you must make one, then you can define `hpi.compatibleSinceVersion` as for xref:plugin-development:mark-a-plugin-incompatible[any plugins].
...
You can also release manually if you have configured your machine for xref:publishing:releasing-manually[manual release].
To cut a release:
...
* You don't have permission to upload to the specified path.
  xref:publishing:requesting-hosting.adoc#request-upload-permissions[Learn more about how to request upload permissions].
