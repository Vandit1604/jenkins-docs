...
Verify that the plugin has already been xref:dev-docs:plugin-development:incrementals.adoc[incrementalified].
...
If you must make one, then you can define `hpi.compatibleSinceVersion` as for xref:dev-docs:plugin-development:mark-a-plugin-incompatible.adoc[any plugins].
