...
See xref:dev-docs:plugin-development:updating-parent.adoc#understanding-requireupperbounddeps-failures-and-fixes[Understanding requireUpperBoundDeps failures and fixes] for more information.
...
. Confirm that you are using the xref:dev-docs:plugin-development:updating-parent.adoc[current parent pom] with the command `mvn versions:update-parent`
