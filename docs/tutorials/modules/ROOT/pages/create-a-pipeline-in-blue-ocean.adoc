...
*Duration:* This tutorial takes 20-40 minutes to complete if you've already met the xref:#prerequisites[prerequisites] below.
...
If you've already run though xref:ROOT:index.adoc[another tutorial], you can skip the xref:#prerequisites[prerequisites] and xref:ROOT:create-a-pipeline-in-blue-ocean.adoc#run-jenkins-in-docker[run Jenkins in Docker] sections below and proceed to xref:#fork-sample-repository[forking the sample repository].
...
. In *Which organization does the repository belong to?*, select your GitHub account where you forked the repository xref:#fork-sample-repository[above].
