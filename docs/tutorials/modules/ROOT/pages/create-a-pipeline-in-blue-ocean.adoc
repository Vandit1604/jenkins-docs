...
The exact duration depends on the speed of your machine and whether or not you've already xref:tutorials:ROOT:create-a-pipeline-in-blue-ocean.adoc.adoc#run-jenkins-in-docker[run Jenkins in Docker] from xref:ROOT:index[another tutorial].
...
If you've already run though xref:ROOT:index.adoc[another tutorial], you can skip the <<prerequisites,prerequisites>> and xref:tutorials:ROOT:create-a-pipeline-in-blue-ocean.adoc#run-jenkins-in-docker[run Jenkins in Docker] sections below and proceed to <<fork-sample-repository,forking the sample repository>>.
...
* The xref:user-docs:ROOT:index.adoc[User Handbook] for more detailed information about using Jenkins, such as xref:user-docs:pipeline:index.adoc[Pipelines], link:/doc/book/pipeline/syntax[Pipeline syntax], and the xref:user-docs:blueocean:index.adoc[Blue Ocean] interface.
