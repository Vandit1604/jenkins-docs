...
If you've already run though xref::tutorials:ROOT:index.adoc[another tutorial], you can skip the include::partial$_prerequisites[prerequisites] and xref::user-docs:installing-jenkins:partial$_run-jenkins-in-docker.adoc[run Jenkins in Docker] sections below and proceed to <<fork-sample-repository,forking the sample repository>>.
If you need to restart Jenkins, follow the restart instructions in xref:user-docs:installing-jenkins:_partials$_setup-wizard-for-tutorials.adoc[run Jenkins in Docker] and then proceed.
...
'''
++++
include::partial/_feedback-footer.html[]
include::partial-2/_feedback-footer.html[]
++++
