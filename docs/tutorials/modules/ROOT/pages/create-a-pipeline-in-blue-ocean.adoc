...
The exact duration depends on the speed of your machine and whether or not you've already xref:user-docs:installing-jenkins:partial$_run-jenkins-in-docker.adoc[run Jenkins in Docker] from xref:ROOT:index.adoc[another tutorial].
...
If you've already run though xref:ROOT:index.adoc[another tutorial], you can skip the <<prerequisites,prerequisites>> and xref:user-docs:installing-jenkins:partial$_run-jenkins-in-docker.adoc[run Jenkins in Docker] sections below and proceed to <<fork-sample-repository,forking the sample repository>>.
