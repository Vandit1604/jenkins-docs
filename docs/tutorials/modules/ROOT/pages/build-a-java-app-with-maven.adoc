...
* The link:/doc/book[User Handbook] for more detailed information about using Jenkins, such as xref:user-docs:pipeline:index.adoc[Pipelines] (in particular xref:user-docs:pipeline:syntax.adoc[Pipeline syntax]) and the link:/doc/book/blueocean[Blue Ocean] interface.
