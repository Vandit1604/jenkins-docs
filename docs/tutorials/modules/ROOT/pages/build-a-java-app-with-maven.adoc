...
<<run-jenkins-in-docker,run Jenkins in Docker>> from xref:index.adoc[another tutorial].
...
<<Prerequisites>> and <<run-jenkins-in-docker,Run Jenkins in
...
* The Maven container becomes the xref:user-docs:glossary:index.adoc#agent[agent] that
