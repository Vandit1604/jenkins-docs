...
The exact duration will depend on the speed of your machine and whether or not you've already <<run-jenkins-in-docker,run Jenkins in Docker>> from xref:index.adoc[another tutorial].

You can stop this tutorial at any time and continue from where you left off.

If you've already run through xref:index.adoc[another tutorial], you can skip the <<prerequisites,Prerequisites>> and <<run-jenkins-in-docker,Run Jenkins in Docker>> sections below and proceed to <<fork-sample-repository,forking the sample repository>>.
...
----
<1> This `image` parameter of the xref:user-docs:pipeline:syntax.adoc#agent[`agent`] section's `docker` parameter downloads the link:https://hub.docker.com/_/maven/[`maven:3.9.5-eclipse-temurin-17-alpine` Docker image], if not already available on your machine, and runs this image as a separate container.
This means that:
