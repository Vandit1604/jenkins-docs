...
If you've already run through xref:index.adoc[another tutorial], you can skip the <<prerequisites,Prerequisites>> and xref:user-docs:installing-jenkins:_partials/_run-jenkins-in-docker.adoc#run-jenkins-in-docker[run Jenkins in Docker] sections below and proceed to <<fork-sample-repository,forking the sample repository>>.
...
include::partial$_run-jenkins-in-docker.adoc[]
