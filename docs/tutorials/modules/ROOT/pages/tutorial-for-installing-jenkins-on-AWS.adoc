...
+
image::AWS/ec2_service.png[Amazon EC2 Services page]

. In the left-hand navigation bar, select **Security Groups**, and then select *Create Security Group*.
+
image::AWS/create_security_group.png[Amazon security group creation screen]
...
+
image::AWS/ec2_launch_instance.png[Launching from Amazon]
...
+
image::AWS/ec2_choose_ami.png[Choosing an Amazon Machine Image]
...
+
image::AWS/ec2_review_instance_launch.png[Review your Amazon launch instance]
...
+
image::AWS/ec2_view_created_instance.png[Amazon view created instance]
...
+
image::AWS/ec2_public_dns.png[Amazon public DNS]
...
+
image::AWS/ec2_putty.png[Amazon EC2 PuTTY selection]
...
+
image::AWS/putty_select_key_pair.png[Selecting and opening a new PuTTY session]
...
+
image::AWS/unlock_jenkins.png[Unlock Jenkins screen]
...
+
image::AWS/create_admin_user.png[Create your first admin user.]
...
without restart*.
+
image::AWS/install_ec2_plugin.png[Jenkins Plugin Manager showing available plugins.]

. Once the installation is done, select *Back to Dashboard*.
. Select *Configure a cloud* if there are no existing nodes or clouds.
+
image::AWS/configure_cloud.png[Jenkins Dashboard showing configure a cloud.]
. If you already have other nodes or clouds set up, select *Manage Jenkins*.
+
image::AWS/manage-jenkins.png[Jenkins dashboard if there are existing clouds.]

.. After navigating to *Manage Jenkins*, select *Configure Nodes and Clouds* from the left hand side of the page.
+
image::AWS/manage-nodes-and-clouds.png[Manage nodes and clouds option from Manage Jenkins page]
.. From here, select *Clouds*.
+
image::AWS/manage-jenkins-configure-clouds.png[Configure clouds option in navigation.]

. Select *Add a new cloud*, and select *Amazon EC2*.
A collection of new fields appears.
+
image::AWS/add-amazon-cloud.png[Adding Amazon EC2 cloud to Jenkins.]

. Click *Add* under Amazon EC2 Credentials
+
image::AWS/configure_cloud_add_ec2_credentials.png[Adding EC2 credentials in Configure Cloud.]
.. From the Jenkins Credentials Provider, select AWS Credentials as the *Kind*.
+
image::AWS/jenkins_credentials_provider_aws_credentials.png[Choosing Kind AWS Credentials.]
.. Scroll down and enter in the IAM User programmatic access keys with permissions to launch EC2 instances and select *Add*.
+
image::AWS/add_access_secret_access_keys.png[Adding AWS Credentials.]
.. Scroll down to select your region using the drop-down, and select *Add* for the EC2 Key Pair's Private Key.
+
image::AWS/configure_cloud_region_private_key.png[Set Region and add Private Key.]
.. From the Jenkins Credentials Provider, select SSH Username with private key as the Kind and set the Username to `ec2-user`.
+
image::AWS/ssh_username.png[Set Kind to SSH Username with private key.]
.. Scroll down and select *Enter Directly* under Private Key, then select *Add*.
+
image::AWS/private_key_enter_directly.png[Set Private Key to Enter Directly.]
...
+
image::AWS/enter_private_key.png[Enter Private Key.]
.. Scroll down to "Test Connection" and ensure it states "Success".
Select *Save* when done
+
image::AWS/test_connection.png[Test Connection.]
...
+
image::AWS/terminate_instance.png[Terminating your AWS EC2 instance.]
