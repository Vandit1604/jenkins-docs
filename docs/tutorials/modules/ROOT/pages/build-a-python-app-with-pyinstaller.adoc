...
*Note:* Unlike the xref:ROOT:index.adoc[other tutorials] in this documentation, this tutorial
...
depend on the speed of your machine and whether or not you've already xref:tutorials:ROOT:create-a-pipeline-in-blue-ocean.adoc#run-jenkins-in-docker[run Jenkins in Docker] from xref:ROOT:index.adoc[another tutorial].

You can stop this tutorial at any point in time and continue from where you left
off.

If you've already run though xref:ROOT:index.adoc[another tutorial], you can skip the <<prerequisites, prerequisites >> and <<run-jenkins-in-docker, Run Jenkins in Docker>> sections below and proceed on to xref:partial$fork-sample-repository.adoc[forking the sample repository]. (Just ensure you have link:https://git-scm.com/downloads[Git] installed locally.) If you need to restart Jenkins, simply follow the restart instructions in xref:user-docs:installing-jenkins:partial$_setup-wizard-for-tutorials.adoc[run Jenkins in Docker] and then proceed on.
