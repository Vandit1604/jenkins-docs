...
*Note:* Unlike the xref:tutorials:index.adoc[other tutorials] in this documentation, this tutorial
...
depend on the speed of your machine and whether or not you've already xref:user-docs:installing-jenkins:partial$_run-jenkins-in-docker.adoc[run Jenkins in Docker] from xref:tutorials:index.adoc[another tutorial].

You can stop this tutorial at any point in time and continue from where you left
off.

If you've already run though xref:tutorials:index.adoc[another tutorial], you can skip the <<prerequisites, prerequisites >> and <<run-jenkins-in-docker, Run Jenkins in Docker>> sections below and proceed on to xref:partial$_fork-sample-repository.adoc[forking the sample repository]. (Just ensure you have link:https://git-scm.com/downloads[Git] installed locally.) If you need to restart Jenkins, simply follow the restart instructions in xref:user-docs:installing-jenkins:partials_setup-wizard-for-tutorials.adoc[run Jenkins in Docker] and then proceed on.
