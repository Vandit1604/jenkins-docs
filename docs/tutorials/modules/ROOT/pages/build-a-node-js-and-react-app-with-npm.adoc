...
xref:tutorials:ROOT:create-a-pipeline-in-blue-ocean.adoc#run-jenkins-in-docker[run Jenkins in Docker] from xref:ROOT:index.adoc[another tutorial].

You can stop this tutorial at any point in time and continue from where you left
off.

If you've already run though xref:ROOT:index.adoc[another tutorial], you can skip the
