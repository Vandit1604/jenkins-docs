...
Docker>> sections below and proceed on to xref:partial$fork-sample-repository.adoc[forking the
...
xref:user-docs:installing-jenkins:partial$_setup-wizard-for-tutorials.adoc[run Jenkins in Docker] and then
proceed on.
